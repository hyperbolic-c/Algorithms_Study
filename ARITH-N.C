/************************** Start of ARITH-N.C *************************
 *
 * This is the order-n arithmetic coding module
 *
 *
 */
#include <stdio.h>
...
void initialize_model( int max_order );
void update_model( int symbol, int max_order );
int convert_int_to_symbol( int symbol, SYMBOL *s );
void get_symbol_scale( SYMBOL *s );
int convert_symbol_to_int( int count, SYMBOL *s );
void add_character_to_model( int c, int max_order );
...
int CompressFile_adaptive_order_n_arithmetic(FILE *input, BIT_FILE *output, int max_order)
{
    SYMBOL s;
    int c;
    int escaped;
	...
	if ( ( ++text_count & 0x0ff ) == 0 )
        ...
        update_model( c, max_order );
        add_character_to_model( c, max_order );
    }
...
int ExpandFile_adaptive_order_n_arithmetic(BIT_FILE *input, FILE *output, int max_order)
{
    SYMBOL s;
    int c;
    int count;
 ...
 *
 * Each context is stored in a special structCONTEXT structure, which is
...
/*
 * The structCONTEXT structure holds all of the known information about
...
void update_table( structCONTEXT *table, int symbol, int max_order );
void rescale_table( structCONTEXT *table );
void totalize_table( structCONTEXT *table );
structCONTEXT *shift_to_next_context( structCONTEXT *table, int c, int order);
structCONTEXT *allocate_next_order_table( structCONTEXT *table,
                                    int symbol,
                                    structCONTEXT *lesser_context );
void recursive_flush( structCONTEXT *table );
...
{
    int i;
    structCONTEXT *null_table;
    structCONTEXT *control_table;

    current_order = max_order;
    contexts = (structCONTEXT **) calloc( sizeof( structCONTEXT * ), 10 );
    if ( contexts == NULL )
        fatal_error( "Failure #1: allocating context table!" );
    contexts += 2;
    null_table = (structCONTEXT *) calloc( sizeof( structCONTEXT ), 1 );
    if ( null_table == NULL )
    ...
    control_table = (structCONTEXT *) calloc( sizeof(structCONTEXT), 1 );
...
structCONTEXT *allocate_next_order_table(structCONTEXT *table, int symbol, structCONTEXT *lesser_context)
{
    structCONTEXT *new_table;
    int i;
    ...
    new_table = (structCONTEXT *) calloc( sizeof( structCONTEXT ), 1 );
    if ( new_table == NULL )
...
void update_model(int symbol, int max_order)
{
    ...
    if ( symbol >= 0 ) {
        while ( local_order <= max_order ) {
            if ( symbol >= 0 )
                update_table( contexts[ local_order ], symbol, max_order );
            local_order++;
        }
    }
    current_order = max_order;
